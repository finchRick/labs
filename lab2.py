...
class Income:
    ...
    def to_file_string(self):
        return f"{self.source} {self.money} {self.date}"
        ...
        with open(filename, 'r') as f:
            for line in f:
                ...
                if type_ == "Rent" and len(parts) >= 5:
                    source, money, tenant, date = parts[1], parts[2], parts[3], parts[4]
                    self.records.append(Rent(source, int(money), tenant, date))
                elif type_ == "Salary" and len(parts) >= 5:
                    source, money, firm, date = parts[1], parts[2], parts[3], parts[4]
                    self.records.append(Salary(source, int(money), firm, date))
    ...
    def save_to_file(self):
        with open("inp.txt", 'w') as f:
            for record in self.records:
                f.write(f"{record.to_file_string()}\n")
